...
def _parseArgs():
    ...
    parser.add_argument("-O", "--output-non-xformed", action="store_true", dest="output_nonxformed", help="In addition to the primary output file (-o|--output-file), also write an intermediate dataset created before performing transormations on the differences and before duplicating+inverting to account for the opposite operand order in the differences. The filename will match that provided to -o|--output-file except that '.intermUnXformed' will be appended to the name (before any 3 letter file extension, if present). [default: do not output this file]", required=False)
    ...
    parser.add_argument("-X", "--output-xformed", action="store_true", dest="output_xformed", help="In addition to the primary output file (-o|--output-file), also write an intermediate dataset created after performing transormations on the differences but before duplicating+inverting to account for the opposite operand order in the differences. The filename will match that provided to -o|--output-file except that '.intermXformed' will be appended to the name (before any 3 letter file extension, if present). [default: do not output this file]", required=False)
...
def _main():
    ...
    if args.output_nonxformed:
        outfn = args.outfn.with_suffix(f".intermUnXformed{args.outfn.suffix}")
        logger.info(f"Writing untransformed, unduplicated intermediate output becuase of -O|--output-non-xformed to {outfn}")
        writeXasDiffDataset (
    ...
    logger.debug(f"StandardScaler will adjust all values: mean->0 with unit std. dev. MinMaxScaler will squish or expand all values until min/max reach provided min/max.")
    ...
    if args.output_xformed:
        ...
        logger.info(f"Writing unduplicated intermediate output becuase of -X|--output-xformed to {outfn}")
        writeXasDiffDataset (
                                pd.concat([X_train,            X_test],           axis="index", ignore_index=True),
                                pd.concat([S_train,            S_test],           axis="index", ignore_index=True),
                                pd.concat([L_train,            L_test],           axis="index", ignore_index=True),
    ...
    #L_train = pd.concat([L_train, pd.DataFrame(R_train.iloc[:,0].values, columns=[L_train.columns[0]])], axis="index", ignore_index=True)
    #L_test  = pd.concat([L_test,  pd.DataFrame(R_test.iloc[:, 0].values, columns=[L_test.columns[0] ])], axis="index", ignore_index=True)
    #R_train = pd.concat([R_train, pd.DataFrame(L_train.iloc[:,0].values, columns=[R_train.columns[0]])], axis="index", ignore_index=True)
    #R_test  = pd.concat([R_test,  pd.DataFrame(L_test.iloc[:, 0].values, columns=[R_test.columns[0] ])], axis="index", ignore_index=True)
    #print(type(L_train))
    #print(type(R_train))
    #print(type(L_test))
    #print(type(R_test))
    ...
    #R_train = pd.concat([R_train, L_train.rename(columns={L_train.columns[0]: R_train.columns[0]}, inplace=False)], axis="index", ignore_index=True)
    #R_test  = pd.concat([R_test,  L_test.rename( columns={L_test.columns[0]:  R_test.columns[0]},  inplace=False)], axis="index", ignore_index=True)
    #L_train_df = L_train.to_frame()
    #L_test_df =  L_test.to_frame()
    #L_train = pd.concat([L_train.to_frame(), R_train.to_frame().rename(   columns={R_train.name:          L_train.name}, inplace=False)], axis="index", ignore_index=True)
    #L_test  = pd.concat([L_test.to_frame(),  R_test.to_frame().rename(    columns={R_test.name:           L_test.name},  inplace=False)], axis="index", ignore_index=True)
    #R_train = pd.concat([R_train.to_frame(), L_train_df.rename(           columns={L_train_df.columns[0]: R_train.name}, inplace=False)], axis="index", ignore_index=True)
    #R_test  = pd.concat([R_test.to_frame(),  L_test_df.rename(            columns={L_test_df.columns[0]:  R_test.name},  inplace=False)], axis="index", ignore_index=True)
    ...
    # write transformed and duplicated train/dev sets to file
    logger.info(f"Writing transformed and duplicated/inverted output to {args.outfn}")
    writeXasDiffDataset (
                            pd.concat([X_train,            X_test],           axis="index", ignore_index=True),
                            pd.concat([S_train,            S_test],           axis="index", ignore_index=True),
                            pd.concat([L_train,            L_test],           axis="index", ignore_index=True),
